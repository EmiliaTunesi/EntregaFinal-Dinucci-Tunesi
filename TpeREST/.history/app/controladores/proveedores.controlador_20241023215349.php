...
class ProveedoresControlador {
    ...
    public function obtenerTodos($req, $res) {
        ...
        if(isset($req->query->orden)) {
                // Convertimos a mayúsculas y validamos
            $ordenRequest = strtoupper($req->query->orden);
            if($ordenRequest === 'DESC') {
                $orden = 'DESC';
            }
        }

        $proveedores = $this->modelo->obtenerTodos($filtrarPorCuit, $ordenarPor, $orden);
    ...
    public function crear($req, $res) {
        if (empty($req->cuerpo->nombre) || empty($req->cuerpo->cuit) || empty($req->cuerpo->ciudad)) {
