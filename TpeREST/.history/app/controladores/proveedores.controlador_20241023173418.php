...
class ProveedoresControlador {
    ...
    public function obtener($req, $res) {
        ...
        $id_proveedor= $req->parametros->id;

        // obtengo la tarea de la DB
        $proveedor = $this->modelo->obtener($id_proveedor);

        if(!$proveedor) {
            return $this->vista->respuesta("El proveedor con el id=$id_proveedor no existe", 404);
        }
    ...
    public function editar($req, $res) {
        ...
        // actualiza la tarea
        $this->modelo->editar($nombre, $contacto, $cuit, $direccion, $informacion_pago);
