...
class ProveedoresApiControlador
{
    ...
    public function obtenerTodos($req)
    {
        ...
        if ($filtrarPor && $valor && !$this->modelo->existeColumna($filtrarPor)) {
            return $this->vista->respuesta(
                "No es posible filtrar por '$filtrarPor', campo inexistente",
                400
            );
        }
        if (!$this->modelo->existeValorEnColumna($filtrarPor, $valor)) {
            return $this->vista->respuesta(
        ...
        if (!$this->modelo->existeColumna($ordenarPor)) {
            return $this->vista->respuesta(
        ...
        if ($ordenar && !in_array($ordenar, ['asc', 'desc'])) {
            return $this->vista->respuesta(
                "El valor de 'ordenar' debe ser 'ascendente' o 'descendente'.", 
                400
            );
        }
